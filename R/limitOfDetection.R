...
limitOfDetection <- function(object, qcThreshold,
    ...
    if(plotType%in%c("scatterplot","MAplot")){
        ...
        ys1 <- rowMeans(cts1,na.rm=TRUE)
        M1 <- ys1-Exp1

        Exp01 <- -log2(((2^-rowMeans(ctspureA))+(2^-rowMeans(ctspureB)))/100)
        ys01 <- rowMeans(cts01,na.rm=TRUE)
        M01 <- ys01-Exp01
        ...
        if(plotType=="scatterplot"){
          ## 0.1/0.1 vs Expected (pure)
            toptick <- ceiling(max(c(Exp1,ys1),na.rm=TRUE)/5)*5
            lowtick <- floor(min(c(Exp1,ys1),na.rm=TRUE)/5)*5
            ...
            toptick <- ceiling(max(c(Exp01,ys01),na.rm=TRUE)/5)*5
            lowtick <- floor(min(c(Exp01,ys01),na.rm=TRUE)/5)*5
            ...
            toptick <- ceiling(max(c(Exp101,ys01),na.rm=TRUE)/5)*5
            lowtick <- floor(min(c(Exp101,ys01),na.rm=TRUE)/5)*5
            if(length(iAllNA01)>0) ys01[iAllNA01] <- toptick+5
          ...
          ## 0.1/0.1 vs Expected (pure)
            toptick <- ceiling(max(M1,na.rm=TRUE)/5)*5
            lowtick <- floor(min(M1,na.rm=TRUE)/5)*5
            ...
            ## 0.01/0.01 vs Expected (pure)
            toptick <- ceiling(max(M01,na.rm=TRUE)/5)*5
            lowtick <- floor(min(M01,na.rm=TRUE)/5)*5
            ...
            ## 0.01/0.01 vs Expected (0.1/0.1)
            toptick <- ceiling(max(M101,na.rm=TRUE)/5)*5
            lowtick <- floor(min(M101,na.rm=TRUE)/5)*5
